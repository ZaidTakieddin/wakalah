...
**NV three-legged flow — FULLY PROVEN Jul 14** (`backend/spike/nv_flow_probe.ps1`): the complete chain ran headless end-to-end — client credentials → OIDC discovery → authorize **302-chain (3 hops, auto-approved: no login page, no mobile connection needed on sandbox; Nokia runs an internal PKCE leg via `camara-auth.eu.nac.nokia.io` with client `testcamara` — only the code delivered to OUR `redirect_uri` is ours to exchange)** → token exchange → Bearer → `POST /verify` returned `{"devicePhoneNumberVerified": true}` for `+99999991000`. Notes: arbitrary `redirect_uri` accepted (no pre-registration on sandbox); access token is **single-use** (a second call with the same token 401s — mint one per verification); `state` round-trips intact. The W2 timebox is now just "wire this sequence into the Mandate Agent."
