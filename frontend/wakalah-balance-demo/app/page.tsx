...
export default function Home() {
  ...
  const syncedTransferIdsRef = useRef(new Set<string>());
  ...
  // Mirror each finished run into the transfer history table exactly once.
  useEffect(() => {
    const newlyFinished = runs.filter(
      (run) => run.status !== "running" && !syncedTransferIdsRef.current.has(run.transactionId),
    );
    if (newlyFinished.length === 0) return;

    newlyFinished.forEach((run) => syncedTransferIdsRef.current.add(run.transactionId));

    setTransfers((current) => [
      ...newlyFinished.map((run) => ({
